...
std::string getVideoDuration(std::string path)
{
    ...
    int duration = stoi(DevGeneral::exec(command));
    int hours = duration / 3600;
    int minutes = (duration - (hours * 3600)) / 60;
    int seconds = duration % 60;
    std::string duration_str = DevGeneral::zeroFill(hours, 2) + ':' + DevGeneral::zeroFill(minutes, 2) + ':' + DevGeneral::zeroFill(seconds, 2);
    return duration_str;
}
...
{
    command = "ffprobe -i \'" + path + "\' -v quiet -show_entries format=duration -hide_banner -of default=noprint_wrappers=1:nokey=1";
    return stoi(DevGeneral::exec(command));
}
